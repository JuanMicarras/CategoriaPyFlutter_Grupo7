...
// --- FAKE ---
class FakeSignUpAuthController extends GetxController
...
void main() {
  ...
  testWidgets('SignUpPage debe renderizar todos los campos y el botón', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('El botón debe estar deshabilitado si faltan campos', (
    WidgetTester tester,
  ) async {
    ...
    final signUpButtonStillDisabled = tester.widget<ElevatedButton>(
      find.byType(ElevatedButton),
    );
    expect(signUpButtonStillDisabled.onPressed, isNull);
  });

  testWidgets('Debe navegar a Login al presionar "Inicia sesión"', (
    WidgetTester tester,
  ) async {
    Get.put<AuthController>(FakeSignUpAuthController());
    ...
    await tester.pumpWidget(
      GetMaterialApp(
        ...
        getPages: [
          ...
          GetPage(
            name: '/login',
            page: () => const Scaffold(body: Text('Vista Login')),
          ),
        ],
      ),
    );
