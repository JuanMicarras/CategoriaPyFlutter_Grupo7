...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => mockController.completedEvaluations,
    ).thenReturn(<String, Map<String, double>>{}.obs);
    when(
      () => mockController.pendingEvaluations,
    ).thenReturn(<String, Map<String, double>>{}.obs);
    when(
      () => mockController.myAverageResults,
    ).thenReturn(<String, double>{}.obs);
    ...
    when(
      () => mockController.loadFormData(any(), any()),
    ).thenAnswer((_) async {});
  ...
  testWidgets('2. Debe mostrar la pantalla de carga cuando isLoading es true', (
    WidgetTester tester,
  ) async {
