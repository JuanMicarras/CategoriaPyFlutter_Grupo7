...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockController.formatStudentName(any(), any()))
        .thenAnswer((inv) => '${inv.positionalArguments[0]} ${inv.positionalArguments[1]}');
    ...
    // Mock de métodos
    when(() => mockController.loadReport(any(), any()))
        .thenAnswer((_) async {});
  ...
  // TEST: RENDER Y CARGA
  // ----------------------------------------------------
  testWidgets('Debe mostrar el título y llamar a loadReport', (WidgetTester tester) async {
  ...
  // ----------------------------------------------------
  testWidgets('Debe mostrar la información del grupo y del estudiante', (WidgetTester tester) async {
    ...
    when(() => mockController.groupReports).thenReturn(<GroupReport>[mockGroup].obs);
